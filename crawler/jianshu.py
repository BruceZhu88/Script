...
def getUrl():
    ...
    while len(userlink) == 0:
        # authorname = input('请输入想查询的作者的名称：')
        authorname = "怀左同学"
        # url = "http://www.jianshu.com/search/do?q=" + authorname +"&type=user&page=1&order_by=default"
        # url = "http://www.jianshu.com/search/do?q=" + authorname +"&page=1&type=note"
        url = "http://www.jianshu.com/search/do?q=%E6%80%80%E5%B7%A6%E5%90%8C%E5%AD%A6&type=note&page=1&order_by=default"
        print(url)
        headers = {
        ...
        'Accept-Encoding' : 'gzip, deflate',
            ...
            'Referer' : 'http://www.jianshu.com/search?q=%E6%80%80%E5%B7%A6%E5%90%8C%E5%AD%A6&page=1&type=note',
            ...
            json_data = requests.post(url = url ,headers = headers).json()
            for i in json_data['related_users']:
                nickname = i['nickname']
                slug = i['slug']
...
def getAuthorinf(page): #得到作者的姓名与头像链接
    ...
    request.urlretrieve(imgurl,'./bg.gif')
...
def get_title(url, titlelist, num):
    """
    读取个人页文章链接
    title[0]: time
    title[1]: link
    ...
    e.g '2017-12-06T09:02:19', '/p/85ee52fdb937', '我跟你奉陪到死吧！——读《老人与海》'
    """
    page = get_page(url)
...
def write_article(articlelist, authorname):
    """
    将文章信息写入txt文件
    """
    print("Write articles to one txt file")
    filePath = './' + authorname + '的文章.txt'
    fileArticle = open(filePath, 'w', encoding='utf-8')
    try:
...
def text_to_wordcloud(articlelist, authorname):
    """
    读取文章生成词云
    """
    stopwords = {}
    ...
    def parse_html_to_pdf(articlelist, authorname):
    """
    将正文转为pdf格式
    """
    html_template = """
    ...
    for index, url in enumerate(articlelist):
        ...
        for html in htmls:
            ...
            def main():    
                ...
                pageNum = 1
                userlink = getUrl()
                titlePage = get_page("http://www.jianshu.com/u/" + userlink)
                ...
                while pageNum <= pageMax :
                    ...
                    articlelist,articleNum= get_title(url,articlelist,articleNum)
                    pageNum +=1
                    time.sleep(3)
                    print('文章链接已读取完毕，马上从链接中读取' + authorname + '的' + str(articleMax) + '篇文章')
                    articlelist = getText(articlelist)
                    write_article(articlelist,authorname)
